...
utr5<-read.csv("20190502_5UTR.csv", row.names=1)
...
for (i in 3169:6555){
	...
	utr5i<-utr5[c(wtname, mtname), c(2:4)]
	for (j in 2:3) utr5i<-cbind(utr5i, utr5[c(wtname, mtname), c((j-1)*4+2:4)])
	count<-as.numeric(as.matrix(t(utr5i)))
...
index.filter<-(3169:6555)[chisqs2<5.99 & chisqs3<5.99]
table(pchisq(chisqs1.filter, df=2, lower.tail=FALSE)<0.05/sum(!is.na(chisqs1.filter)))

hist(pchisq(chisqs1.filter, df=2, low=FALSE))

candidate<-index.filter[order(chisqs1.filter, decreasing=TRUE)[1:19]]

filter5<-na.omit(gtable[index.filter,]) # select stable UTRs
gtable[candidate,]

par(mfrow=c(3,6))
for (k in c(1:8, 10:19 )){

wtname.top<-as.character(gtable[candidate[k],2])
mtname.top<-as.character(gtable[candidate[k],3])

wtc<-as.numeric(as.matrix(utr5[wtname.top, c(2:4, 6:8, 10:12)]))
mtc<-as.numeric(as.matrix(utr5[mtname.top, c(2:4, 6:8, 10:12)]))
